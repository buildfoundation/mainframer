...
if [ -z "$TRAVIS_TAG" ]; then
    echo "Tag is not detected, using version from Cargo.toml."
else
    echo "Tag is detected, overriding version in Cargo.toml..."
    ...
    if ! grep --quiet "$NEW_VERSION" Cargo.toml; then
        echo "Couldn't override version, make sure Cargo.toml and ci/override_version.sh are in sync."
